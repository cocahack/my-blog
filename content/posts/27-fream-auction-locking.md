...
만약 위와 같은 상황에서 제대로 된 동시성 처리가 되지 않는다면 일관성이 깨질 위험이 있다.

### 일관성이 유지될 수 있도록 기능 구현하기

이런 상황을 방지하려면 입찰 등록과 즉시 판매/구매 기능에 락을 사용해야 한다. 락 없이 애플리케이션에서 INSERT 또는 UPDATE를 사용하기 전에 입찰 목록을 조회해서 검사하는 방식으로는 일관성을 유지할 수 없기 때문이다.
...
이 쿼리는 INSERT 또는 UPDATE 전에 실행하여 불변 조건을 확인할 때 사용될 것이다. 따라서 쓰기 락을 걸어서 트랜잭션이 끝나기 전까지 다른 트랜잭션이 건드릴 수 없게 만들어야 한다.
...
위에서 언급했던 두 가지 상황 중 1번 상황에 초점을 맞춰야 한다고 생각했다.
...
불변 조건을 확인하는 역할은 애플리케이션에 구현된 비즈니스 로직이 담당한다고 가정하면, 트랜잭션에서 실행되는 입찰 조회 쿼리는 먼저 조회를 실행한 트랜잭션의 작업이 끝난 뒤에 이뤄져야 불변 조건이 제대로 확인될 것이다.
